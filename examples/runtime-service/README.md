...
This example demonstrates a `RuntimeService` that owns a custom config category, provides a host capability, and has lifecycle hooks.

## Structure

- `wit/greeter.wit`: WIT interface implemented by the host capability
- `host/src/main.rs`: RuntimeService, ConfigHandler, and HostCapability implementation
- `greeter/`: Wasm component that imports the host interface
- `config.toml`: Component, capability, and service configuration

## Building and Running

Change into the `examples/runtime-service` directory.
...
Result: "HOWDY, World!"
[GreetingService] stopped
```

## How It Works

`GreetingService` implements `RuntimeService` with three responsibilities:
...
3. **Lifecycle**: `start()` and `stop()` are called around invocation.
